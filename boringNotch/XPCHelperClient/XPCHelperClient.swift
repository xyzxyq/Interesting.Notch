...
final class XPCHelperClient: NSObject {
    ...
    // The main app owns the CGEvent tap, so TCC must check this process, not XPC.
    nonisolated func requestAccessibilityAuthorization() {
        Task { _ = await ensureAccessibilityAuthorization(promptIfNeeded: true) }
    }

    nonisolated func isAccessibilityAuthorized() async -> Bool {
        await ensureAccessibilityAuthorization(promptIfNeeded: false)
    }

    nonisolated func ensureAccessibilityAuthorization(promptIfNeeded: Bool) async -> Bool {
        await MainActor.run {
            let granted = promptIfNeeded
