...
恢复曲终前收拢过程：最后 2.5 秒开始，以一秒 smoothstep 将圆心半径从 4 缩至 1.5，光芒同时向内收拢、淡去，圆心填充恢复为实心光点。收拢在既有粒子消散前完成，随同一播放时钟支持暂停和回退。中段播放视觉保持不变。

收拢起点、中点、完成、消散开始前及回退断言通过，生产渲染预览补充收拢中间帧并核对；原有运行检查、完整 Debug 构建及严格签名验证通过。应用已重启至 `/tmp/interesting-notch-sun-collapse-build/Build/Products/Debug/boringNotch.app`，保留 `--preview-sun`。

## 统一 2.5 秒转换时长

太阳收拢和月亮转星星共用同一 smoothstep 进度，转换本身持续 2.5 秒：曲终前 3.7 秒开始，曲终前 1.2 秒完成，随后衔接既有粒子消散。修正此前“提前 2.5 秒开始”与“持续 2.5 秒”的区别。极短音轨在可用时长内压缩转换，未知时长不触发。共享进度起点、中点、终点及回退检查通过；已核对生产渲染器的中间帧。

运行检查、完整 Debug 构建及严格签名验证通过，已自动重启 `/tmp/interesting-notch-celestial-duration-build/Build/Products/Debug/boringNotch.app` 并保留太阳预览。

## 四时段天体

自动时段改为凌晨 00–08、白天 08–17、傍晚 17–19、夜间 19–24。凌晨复用月相，降低亮度并添加稳定微星；傍晚绘制逐渐落入短地平线的浅杏色太阳。两者复用现有播放时钟、2.5 秒收尾转换和提前粒子消散。三个独立可插值权重及剩余夜间权重实现任意时段之间的一秒交叉淡入淡出，支持跨午夜和系统时间跳转，不重置歌曲进度。

四个时间边界、UTC+8、本地播放状态和歌词回归检查通过；四时段五阶段生产渲染预览已核对。用户补充的右侧波浪分时段样式目前仅设计，尚未修改。

![四时段天体预览](four-period-preview.png)

完整 Debug 构建与严格签名验证通过，已自动重启 `/tmp/interesting-notch-four-period-build/Build/Products/Debug/boringNotch.app`，无预览覆盖参数，按系统时间自动选择。
